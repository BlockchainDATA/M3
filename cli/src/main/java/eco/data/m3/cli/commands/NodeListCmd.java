...
public class NodeListCmd implements Runnable {
	...
	public void run() {
		...
		ListNodesService req = new ListNodesService();
		ListNodesResponse resp = (ListNodesResponse) client.doRequest(req);
		if(resp!=null)
		{
			int index = 0;
			for (NodeInfo node : resp.getNodes()) {
				System.out.println(">>> Node "+ index++);
				System.out.println(PropertyPrinter.print(node));
			}
			System.out.println("Total "+ resp.getNodes().size() + " Nodes.");
		}
	}
