...
/**
* @Author: xquan
* @Description: Save Node's Info Command. Save node's state.
...
public class NodeSaveCmd implements Runnable {
	...
	public void run() {
		...
		SaveNodeStateService req = new SaveNodeStateService();
		req.setNodeName(nodeName);
		SaveNodeStateResponse resp = (SaveNodeStateResponse) client.doRequest(req);
		if(resp!=null)		
